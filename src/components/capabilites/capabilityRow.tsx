import { capabilityCardSpec, subCapabilitiesCardSpec } from "../../data/capabilities"
...
export default function CapabilityRow({ capability, imageOnLeft }: capabilitesProp) {
    const imagePortion = (
        <div style={{ display: "flex", flexDirection: "column", width: "40%", marginLeft: "52px", marginTop: "32px", marginRight: "32px", marginBottom: "32px", }}>
            <strong style={{ fontSize: "32px" }}>
                {capability.title}
            </strong>
            <div style={{ alignContent: "center", marginTop: "49px", height: "100%", }}>
                <img style={{ objectFit: "cover" }} src={capability.image} alt={capability.title} />
            </div>
        </div>
    )
    ...
    const listPortion = (
        <div style={{ width: "60%", textAlign: "left" }}>
            {capability.subcap.map((it, index) => <SubcapabilityRow subCapability={it} onLeft={imageOnLeft} last={index === capability.subcap.length - 1} />)}
        </div>
    )


    return imageOnLeft ? (
        <div style={{ display: "flex", backgroundColor: "rgba(255, 255, 255, 0.1)" }}>
            {imagePortion}
            {listPortion}
        </div>
    ) : (
